...
| POST | `/register` | guest only | Create account | Creates user, fires `Registered` event (sends verification email), starts session, redirects to `/verify-email`. If the email matches an existing **unverified** account, no new account is created and **the visitor is NOT logged in** (they haven't proven ownership) — a fresh verification email is resent to that address, and the visitor is redirected to `/login` with a flash status message instead. | 422 + validation errors (email already verified by another account, weak password) |
