<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="address_line" name="address_line" type="text" class="mt-1 block w-full" :value="old('address_line', $user->address_line)" required autocomplete="address-line1" />
            <x-input-error class="mt-2" :messages="$errors->get('address_line')" />
        </div>

        <div>
            <x-input-label for="address_complement" :value="__('Apartment / suite / unit (optional)')" />
            <x-text-input id="address_complement" name="address_complement" type="text" class="mt-1 block w-full" :value="old('address_complement', $user->address_complement)" autocomplete="address-line2" />
        ...
        <div>
            <x-input-label for="region" :value="__('State / Province')" />
            <x-text-input id="region" name="region" type="text" class="mt-1 block w-full" :value="old('region', $user->region)" required autocomplete="address-level1" />
        ...
        (function () {
            ...
            function renderSuggestions(suggestions) {
                ...
                suggestions.forEach(function (suggestion) {
                    ...
                    link.textContent = [suggestion.street, suggestion.city, suggestion.region, suggestion.postal_code].filter(Boolean).join(', ');
                    link.addEventListener('click', function () {
                        if (suggestion.street) addressLine.value = suggestion.street;
                        if (suggestion.city) cityInput.value = suggestion.city;
                        if (suggestion.region) regionInput.value = suggestion.region;
